 ...
 *
 * Demo API behaviors:
 * - `GET /api/classes` supports basic search/pagination.
 * - `POST /api/classes` is protected (teacher can create for themselves; admins can create).
 * - `GET /api/classes/:id/users` is protected and enforces roster visibility:
 *   - admin: all
 *   - teacher: only for classes they teach
 *   - student: only if enrolled, and only when querying `role=student`
 */
...
router.post(
  ...
  try {
    ...
    // Teacher users can only create classes for themselves.
...
router.get("/:id/users", requireAuth, async (req, res) => {
  try {
    ...
    // Authorization gate for class rosters.
    // The intent is: avoid leaking roster data across unrelated classes.
    if (req.user!.role !== "admin") {
      if (role === "teacher") {
        // Only teachers (who own the class) can see the teacher roster
        ...
        if (req.user!.role === "teacher") {
          // Teachers can see student roster only for their own class
          if (classRecord.teacherId !== req.user!.id) {
            return res.status(403).json({ error: "Forbidden" });
          }
        }
