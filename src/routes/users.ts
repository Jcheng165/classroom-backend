 ...
 * Users routes.
 *
 * These endpoints support admin/teacher/student dashboards:
 * - `GET /api/users` list users (optional search + role filter)
 * - `GET /api/users/:id` fetch a single user record
 * - `GET /api/users/:id/departments` / `subjects` adapt results based on the user's role
 *
 * Note: The current code paths are designed to be demo-friendly, returning empty
 * arrays for unsupported role combinations rather than hard-failing.
 */
