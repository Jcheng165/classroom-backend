 ...
 *
 * Demo note:
 * - `POST /api/subjects` currently allows creation without auth ("demo bypass")
 *   to avoid friction during early demo flows.
 * - Other subject listing/detail endpoints remain open to support UI rendering.
 *
 * If you re-enable protection later, wire `requireAuth` + `requireRole` and
 * keep the frontend cookie configuration (`credentials: "include"`) intact.
 */
...
// Demo bypass: subject creation is allowed without auth.
